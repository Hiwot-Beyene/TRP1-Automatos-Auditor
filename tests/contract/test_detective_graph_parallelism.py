...
def test_evidence_aggregator_has_edge_to_end():
    """EvidenceAggregator must have a path to END (direct or via evidence_missing_handler)."""
    compiled = build_detective_graph()
    G = _get_graph_structure(compiled)
    end = _end_node(G)
    edge_list = _edges_list(G)
    ...
    for _ in range(5):
        for s, t in edge_list:
            if s in reachable:
                reachable.add(t)
    assert end in reachable, f"evidence_aggregator should have path to END ({end}), got direct succ {succ}"
