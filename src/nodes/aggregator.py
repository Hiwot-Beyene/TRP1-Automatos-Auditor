...
def _in_scope_dimensions(state: dict[str, Any]) -> list[dict[str, Any]]:
    """Get in-scope dimensions.
    
    Uses pre-filtered dimensions from classify_audit node if available,
    otherwise falls back to filtering by active artifacts.
    This ensures context-aware tool selection and eliminates unnecessary processing.
    """
